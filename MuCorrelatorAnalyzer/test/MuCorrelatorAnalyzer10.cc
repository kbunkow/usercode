...
std::ostream & operator<< (std::ostream &out, const l1t::TrackerMuon&  muCand);
...
public:
  ...
  virtual bool accept(const l1t::TrackerMuon& muCorrelatorTrack){
    if(muCorrelatorTrack.hwQual() >= 0) //&& muCorrelatorTrack.getCandidateType() == l1t::TrackerMuon::fastTrack //TODO set the quality cut !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      return true;
    return false;
  }
};
...
public:
  ...
  virtual bool accept(const l1t::TrackerMuon& muCorrelatorTrack){
    if( muCorrelatorTrack.hwQual() >= 12 &&
        muCorrelatorTrack.getCandidateType() == l1t::TrackerMuon::fastTrack &&
...
public:
  ...
  virtual bool accept(const l1t::TrackerMuon& muCorrelatorTrack) {
    if(muCorrelatorTrack.hwQual() >= 12 && muCorrelatorTrack.getCandidateType() == l1t::TrackerMuon::slowTrack)
      return true;
    return false;
  }
};
...
public:
  ...
  virtual bool accept(const l1t::TrackerMuon& muCorrelatorTrack) {
    if( muCorrelatorTrack.hwQual() >= 13 &&
        muCorrelatorTrack.getCandidateType() == l1t::TrackerMuon::slowTrack &&
...
public:
  ...
  virtual bool accept(const l1t::TrackerMuon& muCorrelatorTrack) {
    if( muCorrelatorTrack.getCandidateType() == l1t::TrackerMuon::slowTrack &&
...
std::string toString(const l1t::TrackerMuon&  muCand) {
...
public:
  ...
  virtual void takeCanidate(const l1t::TrackerMuonCollection::const_iterator& itL1MuCand);
...
void AnalyserBase::takeCanidate(const l1t::TrackerMuonCollection::const_iterator& itL1MuCand) {
...
void RateAnalyzer::fillHistos(const edm::Event& event, const edm::Handle< TTTrackAssociationMap< Ref_Phase2TrackerDigi_ > >& MCTruthTTTrackHandle,
  ...
  if(bestL1MuCand != nullptr) {
    muCandPt->Fill(bestL1MuCand->pt()); //filling the data from the l1track_ptr, but this is principle should be the same as in the omtfMuon
    muCandEta->Fill(bestL1MuCand->eta());
    muCandPhi->Fill(bestL1MuCand->phi());

    auto& ttTrackPtr = bestL1MuCand->trkPtr();
...
/*
    ...
    if(tpMatchedToBestL1MuCand.isNonnull() ) {
      if(abs(tpMatchedToBestL1MuCand->pdgId()) == 13) {
        muCandPtMuons->Fill(bestL1MuCand->pt());
        if(bestL1MuCand->pt() >= triggerAlgo->ptCut) {
          //muCandEtaMuons->Fill(bestL1MuCand->getEta());
          //muCandPhiMuons->Fill(bestL1MuCand->getPhi());
        }
      }
      else {
        muCandPtWrongTag->Fill(bestL1MuCand->pt());
        if(bestL1MuCand->pt() >= triggerAlgo->ptCut) {
          //muCandEtaWrongTag->Fill(bestL1MuCand->getEta());
          //muCandPhiWrongTag->Fill(bestL1MuCand->getPhi());

          edm::LogImportant("l1tMuBayesEventPrint")<<"\nrun:lumi:event "<<event.run()<<":"<<event.luminosityBlock()<<":"<<event.id().event()<<endl;
          edm::LogImportant("l1tMuBayesEventPrint")<<" "<<triggerAlgo->name<<" wrongTag muCand track "<<(bestL1MuCand->pt() > 18 ? " high Pt" : "")<<"\n"<<toString(*bestL1MuCand)<<endl;
          bool tmp_trk_genuine = false; //MCTruthTTTrackHandle->isGenuine(ttTrackPtr); isGenuine does not work, crashing!!!!!!!!!!!!!!!!!! TODO
      ...
      muCandPtFakes->Fill(bestL1MuCand->pt());
      if(bestL1MuCand->pt() >= triggerAlgo->ptCut) {
...
/*
      ...
      if(bestL1MuCand->pt() >= triggerAlgo->ptCut) {
        ...
        edm::LogImportant("l1tMuBayesEventPrint")<<"\nrun:lumi:event "<<event.run()<<":"<<event.luminosityBlock()<<":"<<event.id().event()<<endl;
        edm::LogImportant("l1tMuBayesEventPrint")<<" "<<triggerAlgo->name<<" fake muCand track "<<(bestL1MuCand->pt() > 18 ? " high Pt" : "")<<"\n"<<toString(*bestL1MuCand)<<endl;
...
public:
  ...
  public:
    ...
    virtual void fillHistos(const l1t::TrackerMuon& l1MuCand, const edm::Ptr< TrackingParticle >& tpMatchedToL1MuCand, bool passesPtCut) {
      double pt = l1MuCand.pt();
      double eta = l1MuCand.eta();
...
*/
      if(tpMatchedToL1MuCand.isNonnull()) {
        ...
        ptGenDeltaPhi->Fill(tpMatchedToL1MuCand->pt(), tpMatchedToL1MuCand->phi() - l1MuCand.phi() );
      }
    }
  };
  ...
  public:
    ...
    virtual void fillHistos(const l1t::TrackerMuon& l1MuCand, const edm::Ptr< TrackingParticle >& tpMatchedToL1MuCand, bool passesPtCut) {
      ...
      candPt_decayVertexRho->Fill(l1MuCand.pt(), tpMatchedToL1MuCand->decayVertices()[0]->position().rho());
    }
  };
  ...
  void fillHistos(const edm::Event& event, const edm::Handle< TTTrackAssociationMap< Ref_Phase2TrackerDigi_ > >& MCTruthTTTrackHandle,
      const std::vector<edm::Ptr< TrackingParticle > >& muonTrackingParticles, const l1t::TrackerMuon& l1MuCand);
};
...
void MuCandsMatchingAnalyzer::fillHistos(const edm::Event& event, const edm::Handle< TTTrackAssociationMap< Ref_Phase2TrackerDigi_ > >& MCTruthTTTrackHandle,
  ...
  candPt->Fill(l1MuCand.pt());
  candEta->Fill(l1MuCand.eta());
  //muCandPhi->Fill(itL1MuCand->getPhi());

  auto& ttTrackPtr = l1MuCand.trkPtr();
...
*/
  ...
  bool passesPtCut = (l1MuCand.pt() >= triggerAlgo->ptCut);
...
private:
  ...
  //void hscpAnalysis(edm::Handle< std::vector< TrackingParticle > >& trackingParticleHandle, edm::Handle<l1t::TrackerMuonCollection>& muCorrTracksHandle);
  //void hscpAnalysis(const edm::Event& event, edm::Ptr< TrackingParticle >& trackPartPtr, edm::Handle<l1t::TrackerMuonCollection>& muCorrTracksHandle);
...
/*    efficiencyAnalysersCorrelatorWithTrackPart.emplace_back(hscpAlgo20, 20, 100000, fs);
  ...
  muCandsMatchingAnalyzers.emplace_back(std::make_unique<MuCandsMatchingAnalyzer>(singleMuAlgoPtCut0, fs));
...
/*  muCandsMatchingAnalyzers.emplace_back(std::make_unique<MuCandsMatchingAnalyzer>(singleMuAlgoBarrelPtCut10, fs));
  ...
  std::shared_ptr<TriggerAlgo> allTTTRacksPtCut0 = std::make_shared<AllTTTRacks>(0);
  ttTracksMatchingAnalyzer = std::make_unique<MuCandsMatchingAnalyzer>(allTTTRacksPtCut0, fs);
...
void MuCorrelatorAnalyzer::hscpAnalysis(const edm::Event& event, edm::Ptr< TrackingParticle >& trackPartPtr, edm::Handle<l1t::TrackerMuonCollection>& muCorrTracksHandle) {
    ...
    const edm::Event& event, const edm::EventSetup& es)
{
  ...
  LogTrace("l1tMuBayesEventPrint") << "\nMuCorrelatorAnalyzer::"<<__FUNCTION__<<":"<<__LINE__
      <<" MCTruthTTTrackHandle isValid: "<<MCTruthTTTrackHandle.isValid()
      <<" getTTTrackToTrackingParticleMap().size() "<<MCTruthTTTrackHandle->getTTTrackToTrackingParticleMap().size()
      <<" getTrackingParticleToTTTracksMap().size() "<<MCTruthTTTrackHandle->getTrackingParticleToTTTracksMap().size()
      << endl;
...
/*
    ...
    auto printTrackigParticle = [&]() {
      ...
      ostr<< "Tracking particle     pt: " <<setw(7)<< tpPtr->pt() <<" charge: "<<setw(2)<<tpPtr->charge()
                  ...
                  << " beta: " <<setw(7)<< tpPtr->p4().Beta()
                  << " pdgid: " <<setw(7)<< tpPtr->pdgId() << " eventID: " <<setw(7)<< tpPtr->eventId().event()
                  << " ttTracks Cnt " << matchedTracks.size()
                  << " bx: "<<tpPtr->eventId().bunchCrossing()
                  << dec<<" key "<<tpPtr.key()
     ...
     */
    for(auto& matchedTTTrack : matchedTracks) {
      LogTrace("l1tMuBayesEventPrint")<<"L:"<<__LINE__<<" pt "<<matchedTTTrack->momentum().perp()<<" phi "<<matchedTTTrack->phi()<<" "<<endl;
      bool tmp_trk_genuine = false; //MCTruthTTTrackHandle->isGenuine(matchedTTTrack); isGenuine  does not work!!!!!!!!!!!!!!!!!!!!!
      LogTrace("l1tMuBayesEventPrint")<<"L:"<<__LINE__<<endl;
